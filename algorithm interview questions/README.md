...
| DFS或BFS遍历2D网格标记'1'连通组件，统计岛屿数量，变体为计算最大岛屿面积（LC695），适用于图像分割或用户聚类场景，网格规模达10^6需优化空间。 | [LC200 岛屿数量](https://leetcode.cn/problems/number-of-islands/) |
| 二分搜索旋转有序数组，处理偏移点判断单调区间，变体带重复元素（LC81）以模拟噪声数据搜索，O(log n)时间关键。 | [LC33 搜索旋转排序数组](https://leetcode.cn/problems/search-in-rotated-sorted-array/) |
...
| 栈模拟表达式计算，支持+ - * /忽略空格和优先级，变体扩展带括号（LC224），笔试中给示例如“2*3+5/1”考察边界处理。 | [LC227 基本计算器 II](https://leetcode.cn/problems/basic-calculator-ii/) |
| 双端队列维护窗口内递减序列，高效弹出最大值，变体求窗口中位数（LC480）用于实时数据流监控，如用户行为峰值分析。 | [LC239 滑动窗口最大值](https://leetcode.cn/problems/sliding-window-maximum/) |
...
| 遍历价格数组累加所有上涨区间利润，变体添加手续费或单次交易（LC121），模拟推荐算法多次优化场景。 | [LC122 买卖股票的最佳时机 II](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii/) |
...
| 回溯DFS枚举候选数组合和为目标，支持复用，变体无复用（LC40）添加背包容量约束。 | [LC39 组合总和](https://leetcode.cn/problems/combination-sum/) |
...
| 模拟方向遍历矩阵边界，变体逆向生成螺旋矩阵（LC59）。 | [LC54 螺旋矩阵](https://leetcode.cn/problems/spiral-matrix/) |
